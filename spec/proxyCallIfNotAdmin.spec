...
rule AdminConsistencyAsRule( env e, method f) {
    // Precondition
    require owner(e.msg.sender) == admin(e.msg.sender);
    ...
    // Postcondition
    assert owner(e) == admin(e),
        "Admin should be equal to admin";
}
// Additional rule to check admin consistency during potential reentrancy
rule admin_consistency_during_reentrancy(env e, method f) filtered {f -> !f.isView} {
    calldataarg args; 
    address admin_before = admin(e.msg.sender);
    f@withrevert(e, args);
    address admin_after = admin(e.msg.sender);
